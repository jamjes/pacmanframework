...
public class Pathfinding
{
    ...
    public Pathfinding(Transform self, LayerMask wallLayer) { 
        this.self = self;
    ...
    public Vector2[] GetAvailableDirections(Vector2 currentDirection) {
        List<Vector2> directions = new List<Vector2>();

        RaycastHit2D hit = Physics2D.Raycast(self.position, Vector2.right, .55f, wallLayer);
        if (hit.collider == null && currentDirection * -1 != Vector2.right) {
            directions.Add(Vector2.right);
        }

        hit = Physics2D.Raycast(self.position, Vector2.down, .55f, wallLayer);
        if (hit.collider == null && currentDirection * -1 != Vector2.down) {
            directions.Add(Vector2.down);
        }

        hit = Physics2D.Raycast(self.position, Vector2.left, .55f, wallLayer);
        if (hit.collider == null && currentDirection * -1 != Vector2.left) {
            directions.Add(Vector2.left);
        }

        hit = Physics2D.Raycast(self.position, Vector2.up, .55f, wallLayer);
    ...
    public Vector2 GetShortestDirection(Vector2 targetPosition, Vector2 currentDirection) {
        Vector2[] availableDirections = GetAvailableDirections(currentDirection);
